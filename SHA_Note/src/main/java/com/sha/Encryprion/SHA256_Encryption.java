...
import com.sha.Encryprion.SHA256_INFO;
import com.sha.Encryprion.Common;
...
public class SHA256_Encryption {
	...
	public static String encrypt(String plainText) {
		...
		Encryption(plainText.getBytes(), plainText.getBytes().length, encryptByte);
		
		for (int i=0; i<32; i++)
			encryptStringBuffer.append(encrpytData(Integer.toHexString(0xff&encryptByte[i])));
		
		return encryptStringBuffer.toString();
	}
	
	private static Object encrpytData(String hexString) {
	...
	*/
	public static void Encryption( byte[] pszMessage, int uPlainTextLen, byte[] pszDigest ) {
		SHA256_INFO info = new SHA256_INFO(new int[SHA256_DIGEST_VALUELEN / 4], new byte[SHA256_DIGEST_BLOCKLEN]);
		SHA256_Init( info );
